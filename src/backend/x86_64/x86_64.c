...
struct register_list_t *x86_64_create_register_list(struct arena *arena, struct codegen_build_target_t *arch) {
...
struct register_t *x86_64_get_reg_with_arg_index(struct register_list_t *list, int arg_index) {
...
const char *x86_64_get_register_name(struct register_list_t *list, struct register_t *reg, enum register_size size) {
...
static inline void calc_preferred_regs(struct register_list_t *list, struct IR_Operand *operand, struct IR_Instruction *instruction, struct bitset_t *preferred_regs) {
    switch (instruction->type) {
        ...
        case IR_INSTRUCTION_TYPE_DIVIDE: {
            ...
            bitset_set(preferred_regs, x86_64_get_reg_with_arg_index(list, arg_index)->id);
            break;
...
void x86_64_get_preferred_registers(struct register_list_t *list, struct IR_Operand *operand, struct bitset_t *preferred_regs) {
    if (!list) return;
...
void x86_64_set_free_reserved_register(struct register_list_t *list, struct register_t *reg) {
    if(!reg || !list) return;
    if(!reg->is_reserved) return;
    reg->is_reserved_reg_busy = false;
}
struct register_t *x86_64_get_available_reserved_register(struct register_list_t *list) {
    for(int i = 0;i < list->register_count; ++i) {
...
int x86_64_get_reg_in_reg_preference_order(int index) {
    return REG_PREFERENCE_ORDER[index];
}
struct register_t *x86_64_get_best_available_register(struct register_list_t *list, struct register_t *preferred_register, struct vector_t *clobber_list, struct IR_Operand *vreg) {
...
// emit
void x86_64_emit_mov_reg_to_reg(struct codegen_context_t *context, struct register_t *dest, struct register_t *src, enum register_size size) {
...
void x86_64_emit_mov_arg_to_stack_offset(struct codegen_context_t *context, int offset, bool is_argument, struct IR_Operand *src) {
    struct register_t *reg = x86_64_ensure_operand_is_register(context, src);
    ...
    x86_64_emit_reg(context, reg, REGISTER_SIZE_64, false);
    codegen_emit(context->file, "\n");


    x86_64_set_free_reserved_register(context->build_target->registers, reg);
}

void x86_64_emit_movsx_reg_to_reg(struct codegen_context_t *context, struct register_t *dest, struct register_t *src, enum register_size dest_size, enum register_size src_size) {
    codegen_emit(context->file, "    movsx ");
    x86_64_emit_reg(context, dest, dest_size, true);
    codegen_emit(context->file, ", ");
    x86_64_emit_reg(context, src, src_size, true);
    codegen_emit(context->file, "\n");
}
void x86_64_emit_movsx_operand_to_reg(struct codegen_context_t *context, struct register_t *dest, struct IR_Operand *src, enum register_size dest_size) {
    codegen_emit(context->file, "    movsx ");
    x86_64_emit_reg(context, dest, dest_size, true);
    codegen_emit(context->file, ", ");
    x86_64_emit_operand(context, src, codegen_get_register_size_from_operand(src), true, true, 0);
    codegen_emit(context->file, "\n");
}
void x86_64_emit_mov_operand_to_reg(struct codegen_context_t *context, struct register_t *dest, struct IR_Operand *src) {
    ...
    if(IR_OPERAND_TYPE_STACK_SLOT == src->type) {
        ...
        x86_64_emit_reg(context, dest, src_size, true);
        codegen_emit(context->file, ", ");
        x86_64_emit_operand(context, src, src_size, true, true, 0);
        ...
        x86_64_emit_reg(context, dest, dest_size, true);
        codegen_emit(context->file, ", ");
        x86_64_emit_operand(context, src, dest_size, true, true, 0);
        codegen_emit(context->file, "#op (imm) to reg\n");
        return;
    }else if(IR_OPERAND_TYPE_VREG == src->type) {
        x86_64_emit_mov_reg_to_reg(context, dest, src->data.vreg.reg, src_size);
        return;
...
void x86_64_emit_mov_reg_to_operand(struct codegen_context_t *context, struct IR_Operand *dest, struct register_t *src) {
    enum register_size size = codegen_get_register_size_from_operand(dest);

    if(IR_OPERAND_TYPE_VREG == dest->type) {
        x86_64_emit_mov_reg_to_reg(context, dest->data.vreg.reg, src, size);
        return;
    }else if(IR_OPERAND_TYPE_STACK_SLOT == dest->type) {
        codegen_emit(context->file, "    mov ");
        x86_64_emit_operand(context, dest, size, true, true, 0);
        codegen_emit(context->file, ", ");
        x86_64_emit_reg(context, src, size, true);
...
void x86_64_emit_mov_operand_to_operand(struct codegen_context_t *context, struct IR_Operand *dest, struct IR_Operand *src) {
    if(dest == src) return;
    ...
    if(IR_OPERAND_TYPE_STACK_SLOT == dest_type && IR_OPERAND_TYPE_VREG != src_type) {
        struct register_t *reserved_reg = x86_64_get_available_reserved_register(context->build_target->registers);
        x86_64_emit_mov_operand_to_reg(context, reserved_reg, src);
        x86_64_emit_mov_reg_to_operand(context, dest, reserved_reg);
        x86_64_set_free_reserved_register(context->build_target->registers, reserved_reg);
        return;
    }else if(IR_OPERAND_TYPE_STACK_SLOT == dest_type && IR_OPERAND_TYPE_VREG == src_type) {
        x86_64_emit_mov_reg_to_operand(context, dest, src->data.vreg.reg);
        return;
    }else if(IR_OPERAND_TYPE_VREG == dest_type && IR_OPERAND_TYPE_VREG == src_type) {
        x86_64_emit_mov_reg_to_reg(context, dest->data.vreg.reg, src->data.vreg.reg, src_size);
        return;
    }else if(IR_OPERAND_TYPE_VREG == dest_type && IR_OPERAND_TYPE_VREG != src_type) {
        x86_64_emit_mov_operand_to_reg(context, dest->data.vreg.reg, src);
        return;
...
static inline int push_or_pop_registers(struct codegen_context_t *context, struct bitset_t *registers, struct bitset_t *registers2, struct bitset_t *mask, enum register_size size, const char *instruction, bool reverse, bool nop) {
    ...
    for (int i = start; i != end; i += step) {
        ...
        if (!nop) {
            const char *caller_saved_reg = x86_64_get_register_name(
...
struct register_t *x86_64_ensure_operand_is_register(struct codegen_context_t *context, struct IR_Operand *operand) {
    if(IR_OPERAND_TYPE_VREG == operand->type) return operand->data.vreg.reg;

    struct register_t *reserved_reg = x86_64_get_available_reserved_register(context->build_target->registers);
    x86_64_emit_mov_operand_to_reg(context, reserved_reg, operand);
    return reserved_reg;
}

void x86_64_emit_global_type(struct codegen_context_t *context, struct IR_Operand *global) {
...
void x86_64_emit_globals(struct codegen_context_t *context, bool jmp_to_main) {
   ...
   x86_64_emit_label(context, SV("_start"), true);
   if(jmp_to_main) x86_64_emit_jmp_main(context);
}

void x86_64_emit_jmp_main(struct codegen_context_t *context) {
...
struct str_view x86_64_get_label_str(struct codegen_context_t *context, const struct str_view label, bool is_global) {
    return label;
}

void x86_64_emit_label(struct codegen_context_t *context, const struct str_view label, bool is_global) {
    if(is_global) {
        codegen_emit(context->file, SV_FMT ":\n", SV_ARG(label));
        return;
    }
...
void x86_64_emit_function_prologue(struct codegen_context_t *context, struct IR_Function *function) {
    context->build_target->emit_label(context, function->mangled_name, true);
...
void x86_64_emit_function_epilogue(struct codegen_context_t *context, struct IR_Function *function) {
...
void emit_cast_instruction(struct codegen_context_t *context, struct IR_Instruction *instruction) {
    ...
    if(dest_size > src_size) {
        struct register_t *dest_reg = x86_64_ensure_operand_is_register(context, dest);
        struct register_t *src_reg = x86_64_ensure_operand_is_register(context, src);

        codegen_emit(context->file, "    movsx ");
        x86_64_emit_reg(context, dest_reg, dest_size, false);
        codegen_emit(context->file, ", ");
        x86_64_emit_reg(context, src_reg, src_size, false);
        codegen_emit(context->file, "\n");

        x86_64_emit_mov_reg_to_operand(context, dest, dest_reg);

        x86_64_set_free_reserved_register(context->build_target->registers, src_reg);
        x86_64_set_free_reserved_register(context->build_target->registers, dest_reg);
        return;
    }

    x86_64_emit_mov_operand_to_operand(context, dest, src);
}
...
void emit_comparison_instructions(struct codegen_context_t *context, struct IR_Instruction *instruction) {
    ...
    struct register_t *src1_reg = x86_64_ensure_operand_is_register(context, src1);
    ...
    x86_64_emit_reg(context, src1_reg, size, true);
    codegen_emit(context->file, ", ");
    x86_64_emit_operand(context, src2, size, true, true, 0);
    codegen_emit(context->file, "\n");

    codegen_emit(context->file, "    %s ", instruction_str);
    x86_64_emit_reg(context, dest_reg, REGISTER_SIZE_8, true);
    codegen_emit(context->file, "\n");

    // codegen_emit(context->file, "    movzx ");
    // x86_64_emit_reg(context, dest_reg, REGISTER_SIZE_64, true);
    // codegen_emit(context->file, ", ");
    // x86_64_emit_reg(context, dest_reg, REGISTER_SIZE_8, true);
    // codegen_emit(context->file, "\n");
    ...
    x86_64_set_free_reserved_register(context->build_target->registers, src1_reg);
    x86_64_set_free_reserved_register(context->build_target->registers, dest_reg);
}
...
void emit_arithmetic_instructions(struct codegen_context_t *context, struct IR_Instruction *instruction) {
    ...
    if(IR_INSTRUCTION_TYPE_MUL == instruction->type) {
        if(REGISTER_SIZE_8 == size) {  // imul source_2
            ...
            x86_64_emit_mov_operand_to_reg(context, rax, src1);

            struct register_t *src2_reg = x86_64_ensure_operand_is_register(context, src2);
            codegen_emit(context->file, "    %s ", instruction_str);
            x86_64_emit_reg(context, src2_reg, size, false);
            codegen_emit(context->file, "    \n");

            x86_64_emit_mov_reg_to_operand(context, dest, rax);

            x86_64_set_free_reserved_register(context->build_target->registers, src2_reg);
            goto exit;
        }
        ...
        if(dest == src1) { // imm dest, src2
            if(IR_OPERAND_TYPE_STACK_SLOT == dest_type) {
                ...
                codegen_emit(context->file, "    %s ", instruction_str);
                x86_64_emit_operand(context, dest, size, false, true, 0);
            ...
            x86_64_emit_operand(context, dest, size, false, true, 0);
            codegen_emit(context->file, ", ", instruction_str);
            x86_64_emit_operand(context, src2, size, false, true, 0);
            codegen_emit(context->file, "\n", instruction_str);
            goto exit;
        }
        ...
        if(IR_OPERAND_TYPE_VREG == dest_type && (IR_OPERAND_TYPE_VREG == src1_type || IR_OPERAND_TYPE_STACK_SLOT == src1_type) && (IR_OPERAND_TYPE_IMM == src2_type && src2->type_info->size <= 4)) { // imm dest, src1, src2
            ...
            codegen_emit(context->file, "    %s ", instruction_str);
            x86_64_emit_reg(context, dest_reg, size, false);
            codegen_emit(context->file, ", ", instruction_str);
            x86_64_emit_operand(context, src1, size, false, true, 0);
            codegen_emit(context->file, ", ", instruction_str);
            x86_64_emit_operand(context, src2, size, false, true, 0);
            codegen_emit(context->file, "\n", instruction_str);
        ...
        x86_64_emit_mov_operand_to_reg(context, dest_reg, src1);
        ...
        if(dest != src1) {
            x86_64_emit_mov_operand_to_operand(context, dest, src1);
        }
        if (original_size == REGISTER_SIZE_8 && size == REGISTER_SIZE_16) {
            x86_64_emit_movsx_operand_to_reg(context, rax, dest, REGISTER_SIZE_16);
        } else {
            x86_64_emit_mov_operand_to_reg(context, rax, dest);
        }
        // x86_64_emit_mov_operand_to_reg(context, rax, dest);
        ...
        if (original_size == REGISTER_SIZE_8 && size == REGISTER_SIZE_16) {
            x86_64_emit_movsx_reg_to_reg(context, src2_reg, src2_reg, REGISTER_SIZE_16, original_size);
        }
        ...
        x86_64_emit_mov_reg_to_operand(context, dest, res_reg);
        goto exit;
    }
    ...
    if(instruction->type == IR_INSTRUCTION_TYPE_SHL || instruction->type == IR_INSTRUCTION_TYPE_SHR) {
        if(src2 && src2_type != IR_OPERAND_TYPE_IMM) {
            ...
            const char *cl_str = x86_64_get_register_name(context->build_target->registers, cl_reg, REGISTER_SIZE_8);
            x86_64_emit_mov_operand_to_operand(context, dest, src1);
            x86_64_emit_mov_operand_to_reg(context, cl_reg, src2);
...
void x86_64_emit_instruction(struct codegen_context_t *context, struct IR_Instruction *instruction) {
    switch (instruction->type) {
        case IR_INSTRUCTION_TYPE_MOV: {
            x86_64_emit_mov_operand_to_operand(context, instruction->operands.double_operands.destination, instruction->operands.double_operands.source_1);
            break;
            ...
            codegen_emit(context->file, "    mov ");
            x86_64_emit_reg(context, dest_reg, size, true);
            codegen_emit(context->file, ", [");
            x86_64_emit_reg(context, src_reg, REGISTER_SIZE_64, true);
            codegen_emit(context->file, "]\n");
        ...
        case IR_INSTRUCTION_TYPE_STORE: {
            ...
            x86_64_emit_reg(context, dest_reg, REGISTER_SIZE_64, true);
            codegen_emit(context->file, "], ");
            x86_64_emit_reg(context, src_reg, size, true);
            codegen_emit(context->file, "\n");
            ...
            struct IR_Operand *dest       = instruction->operands.triple_operands.destination;
            struct IR_Operand *array_addr = instruction->operands.triple_operands.source_1;
            struct IR_Operand *index      = instruction->operands.triple_operands.source_2;
            ...
            if(index->type == IR_OPERAND_TYPE_IMM) {
                ...
                codegen_emit(context->file, "    lea ");
                x86_64_emit_reg(context, dest_reg, dest_size, true);
                codegen_emit(context->file, ", [");

                if (array_addr->type == IR_OPERAND_TYPE_STACK_SLOT) {
                    x86_64_emit_operand(context, array_addr, dest_size, false, false, index_imm);
                } else {
                    x86_64_emit_operand(context, array_addr, dest_size, false, false, 0);
                    if (index_imm > 0) {
                ...
                struct register_t *index_reg = x86_64_ensure_operand_is_register(context, index);
                ...
                if (stride == 1 || stride == 2 || stride == 4 || stride == 8) {
                    ...
                    codegen_emit(context->file, " + ");
                    x86_64_emit_reg(context, index_reg, dest_size, false);
                ...
                x86_64_set_free_reserved_register(context->build_target->registers, index_reg);
            }
        ...
        case IR_INSTRUCTION_TYPE_RET: {
            struct register_t *rax = context->build_target->registers->registers + X86_64_RAX;
            x86_64_emit_mov_operand_to_reg(context, rax, instruction->operands.ret.return_value);
            break;
        ...
        case IR_INSTRUCTION_TYPE_UNARY_NOT: {
            ...
            if(IR_OPERAND_TYPE_IMM == src->type) {
                ...
                x86_64_emit_reg(context, reserved_reg, size, true);
                codegen_emit(context->file, "\n");
                x86_64_emit_mov_reg_to_operand(context, dest, reserved_reg);
            ...
            codegen_emit(context->file, "    lea ");
            x86_64_emit_reg(context, dest_reg, REGISTER_SIZE_64, false);
            codegen_emit(context->file, ", ");
            x86_64_emit_operand(context, src, src_size, false, true, 0);
            codegen_emit(context->file, "\n");
            ...
            struct register_t *val_reg = x86_64_get_available_reserved_register(context->build_target->registers);

            codegen_emit(context->file, "    mov ");
            x86_64_emit_reg(context, val_reg, size, true);
            codegen_emit(context->file, ", [ ");
            x86_64_emit_reg(context, src_reg, REGISTER_SIZE_64, true);
            codegen_emit(context->file, " ]\n");

            x86_64_emit_mov_reg_to_operand(context, dest, val_reg);

            x86_64_set_free_reserved_register(context->build_target->registers, val_reg);
            x86_64_set_free_reserved_register(context->build_target->registers, src_reg);
            break;
        }case IR_INSTRUCTION_TYPE_CALL: {
            ...
            for (int i = arg_count - 1; i >= 6; --i) {
                ...
                x86_64_emit_mov_arg_to_stack_offset(context, offset, true, arg);
            }
            ...
            for(int i = 0;i < args_via_registers; ++i) {
                struct register_t *reg = x86_64_get_reg_with_arg_index(context->build_target->registers, i);
            ...
            codegen_emit(context->file, "    call " SV_FMT "\n", SV_ARG(context->build_target->get_label(context, func->mangled_name, true)));
            ...
            struct register_t *rax = context->build_target->registers->registers + X86_64_RAX;
            x86_64_emit_mov_reg_to_operand(context, return_val, rax);
            break;
            ...
            if(args) {
                ...
                struct vector_t *args_to_graph = vector_create_vector(context->codegen->temp_arena, arg_count, sizeof(struct IR_Operand *));
                ...
                for(int i = 0;i < arg_count; ++i) {
                    ...
                    if(op->type != IR_OPERAND_TYPE_VREG) {
                        x86_64_emit_mov_operand_to_operand(context, op, arg);
                        continue;
                    }
                    vector_add(args_to_graph, &arg);
                    vector_add(out_regs, &(op->data.vreg.reg));
                }
                if(args_to_graph)codegen_utils_emit_call_args(context->codegen->temp_arena, context, args_to_graph, out_regs, args_to_graph->element_count);
            }
            ...
            {   // false block
                ...
                codegen_emit(context->file, "    cmp ");
                x86_64_emit_operand(context, condition, codegen_get_register_size_from_operand(condition), true, true, 0);
...
void x86_64_emit_reg_size(struct codegen_context_t *context, enum register_size size) {
    switch (size) {
...
void x86_64_emit_reg(struct codegen_context_t *context, struct register_t *reg, enum register_size size, bool print_size) {
...
void x86_64_emit_operand(struct codegen_context_t *context, struct IR_Operand *op, enum register_size size, bool print_size, bool print_brackets, size_t offset) {
    ...
    switch (op->type) {
        ...
        case IR_OPERAND_TYPE_VREG: {
            ...
            x86_64_emit_reg(context, op->data.vreg.reg, size, print_size);
            break;
            ...
            if(print_size) {
                x86_64_emit_reg_size(context, codegen_calc_register_size(slot->type->size));
            ...
            codegen_emit(context->file, "%srip + "SV_FMT "%s", l_bracket, SV_ARG(op->data.global.name), r_bracket);
            break;
        }
