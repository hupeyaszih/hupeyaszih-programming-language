...
struct codegen_t *codegen_create_codegen(struct parser_t *restrict parser, const char *restrict out_filename) {
    ...
    if (NULL == cg) {
        LOG_M_ERR("Codegen memory allocation failed");
        return NULL;
    }
    ...
    if (NULL == cg->output_file) {
        C_LOG_ERR("Could not open output file: %s", out_filename);
        cg->successful = 0;
...
void codegen_generate(struct codegen_t *restrict codegen, struct parser_t *restrict parser, struct symbol_table *global_scope){
    if(NULL == codegen) {
        return;
    }else if(NULL == codegen || NULL == parser || NULL == global_scope){
        codegen->successful = 0;
        return;
    }else if(0 == codegen->successful) {
        return;
    }
    ...
    fprintf(codegen->output_file, "; --- Generated by Hupeyaszih Compiler ---\n");
    fprintf(codegen->output_file, "section .text\n");
    fprintf(codegen->output_file, "    global _start\n\n");
    ...
    if(codegen->successful == 1){
        C_LOG_OK("Codegen finished successfully");
    }else {
        C_LOG_ERR("Codegen failed");
    }
}
...
void codegen_visit_node(struct codegen_t *restrict codegen, struct parser_node *restrict node, struct symbol_table *global_scope, int flush_mode) {
    ...
    break;
        ...
        case PARSER_NODE_CALL: {
            ...
            if(NULL == node->data.loop.body) {
                C_LOG_ERR("Function body is not defined, line: %d", node->line);
                codegen->successful = 0;
                return;
            }
            ...
            if(NULL == node->data.loop_control.body || NULL == node->data.loop_control.condition) {
                codegen->successful = 0;
                return;
            }
            ...
            if(NULL == symbol) {codegen->successful = 0; C_LOG_ERR("variable \"%s\" is not defined, line: %d", node->data.variable_name, node->line); break;}
            fprintf(codegen->output_file, "    mov [rbp-%d], rax\n", symbol->stack_offset);
            break;
        }case PARSER_NODE_VARIABLE_ASSIGMENT:{
            if (NULL == node->left_node || NULL == node->right_node) {
                codegen->successful = 0;
                return;
            }
            if (node->right_node) {
                codegen_visit_node(codegen, node->right_node, global_scope, 0);
            }
            fprintf(codegen->output_file, "    push rax\n");

            codegen->is_lvalue = 1; 
            codegen_visit_node(codegen, node->left_node, global_scope, 0); 
            codegen->is_lvalue = 0;

            fprintf(codegen->output_file, "    pop r10\n");

            fprintf(codegen->output_file, "    mov [rax], r10\n");
            break;
        }
        case PARSER_NODE_IDENTIFIER:{
            struct symbol_t *symbol = symbol_table_look_up(codegen->current_scope, node->data.variable_name);
            if(NULL == symbol) {codegen->successful = 0; C_LOG_ERR("variable \"%s\" is not defined, line: %d", node->data.variable_name, node->line); break;}
            if (codegen->is_lvalue) {
                fprintf(codegen->output_file, "    lea rax, [rbp - %d]\n", symbol->stack_offset);
            } else {
        ...
        case PARSER_NODE_UNARY_ADDRESS_OF:
            if (node->right_node->type == PARSER_NODE_IDENTIFIER) {
                struct symbol_t *sym = symbol_table_look_up(codegen->current_scope, node->right_node->data.variable_name);
                sym->type = type_table_get_type_info(codegen->type_table, "ptr");
        ...
        case PARSER_NODE_UNARY_DEREFERENCE:
            ...
            codegen_visit_node(codegen, node->right_node, global_scope, 0);
            fprintf(codegen->output_file, "    mov rax, [rax]\n");
            break;
...
void codegen_calculate_offsets(struct symbol_table *scope, int base_offset) {
    ...
    for (int i = 0; i < scope->symbol_count; i++) {
        ...
        if (sym->kind == SYMBOL_KIND_VARIABLE) {
            int size_padding = type_table_size_padding(sym->type->size);
